...
async function handleWithdraw(supabase: any, member: any, amount: number, chamaId: string, paymentMethod: string, recipient: string) {
  ...
  // If internal transfer, add to user's central wallet
  if (paymentMethod === 'internal') {
    ...
    if (walletError && walletError.code !== 'PGRST116') {
      throw new Error('Failed to access central wallet');
    }
    ...
    await supabase
      ...
      .insert({
        ...
        type: 'chama_transfer',
        amount: amount,
        description: 'Transfer from MGR wallet',
        status: 'completed'
      });
  }
  ...
  await supabase
    ...
    .insert({
      chama_id: chamaId,
      transaction_type: paymentMethod === 'internal' ? 'internal_transfer' : 'withdrawal',
      amount: amount,
      description: paymentMethod === 'internal' ? 'Transfer to internal wallet' : `Withdrawal to ${paymentMethod} (${recipient})`,
      processed_by: member.id,
  ...
  await supabase
    ...
    .insert({
      chama_id: chamaId,
      member_id: member.id,
      activity_type: paymentMethod === 'internal' ? 'internal_transfer' : 'withdrawal',
      description: paymentMethod === 'internal' ? `Transferred KES ${amount} to internal wallet` : `Withdrew KES ${amount} to ${paymentMethod}`,
      amount: amount
    });

  return {
    message: paymentMethod === 'internal' ? `Transfer of KES ${amount} to internal wallet completed` : `Withdrawal of KES ${amount} processed successfully`,
